...
import { useOperationsFindFirst, useRoqClient } from 'lib/roq';

import { OperationsInterface } from 'interfaces/operations';

function OperationsViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { user: boolean; car: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useOperationsFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('operations', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/operations/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Task" text={data?.task} />
