...
import { useRoqClient, useOperationsFindFirst } from 'lib/roq';
...
function OperationsEditPage() {
  ...
  const { data, error, isLoading, mutate } = useOperationsFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: OperationsInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.operations.update({
        data: values as RoqTypes.operations,
        where: {
          id,
        },
      });
  ...
  const formik = useFormik<OperationsInterface>({
    initialValues: data,
