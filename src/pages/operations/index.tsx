...
import { useRoqClient, useOperationsFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<OperationsInterface, unknown>;
...
export function OperationsListPage(props: OperationsListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['task.contains', 'status.contains'],
          ...params.filters,
        },
        'operations',
      ),
  ...
  const { data, error, isLoading, mutate } = useOperationsFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.operations.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: OperationsInterface) => {
    if (hasAccess('operations', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/operations/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/operations/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('operations', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/operations/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('operations', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="operations" />
        </Flex>

        {hasAccess('operations', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/operations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
